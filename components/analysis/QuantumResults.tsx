...
export default function QuantumResults({ results }: QuantumResultsProps) {
  ...
  const optimizationData = results.optimizations.map(
    (opt: any, idx: number) => ({
      name: opt.parameter.split(" ")[0],
      improvement: parseFloat(
        opt.improvement.replace("+", "").replace("%", "")
      ),
    })
  );
  ...
  return (
    ...
    >
      <div className="flex items-center mb-4">
        ...
        <h3 className="text-2xl font-bold text-gray-800">Quantum Analysis</h3>
      </div>
      ...
      <div className="mb-4">
        ...
        <p className="text-gray-700 text-sm">
          {results.qubits} qubits • {results.speedup} speedup
        </p>
      </div>
          ...
          <div className="space-y-2">
            <div className="bg-white/10 rounded-lg p-2">
              <div className="flex justify-between mb-1">
                <span className="text-gray-800 text-xs">Success</span>
              ...
              <div className="w-full bg-white/20 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${results.probabilities.successProbability * 100}%`,
                  }}
                />
              </div>
            </div>
            <div className="bg-white/10 rounded-lg p-2">
              <div className="flex justify-between mb-1">
                <span className="text-gray-800 text-xs">Opportunity</span>
                <span className="text-gray-800 text-xs font-semibold">
                  {(results.probabilities.opportunityProbability * 100).toFixed(
                    1
                  )}
                  %
                </span>
              ...
              <div className="w-full bg-white/20 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${
                      results.probabilities.opportunityProbability * 100
                    }%`,
                  }}
                />
              </div>
