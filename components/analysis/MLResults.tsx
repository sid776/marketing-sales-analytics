...
export default function MLResults({ results }: MLResultsProps) {
  const COLORS = ['#3b82f6', '#fbbf24', '#60a5fa', '#fcd34d', '#93c5fd', '#fef08a']
  ...
  return (
    ...
    >
      <div className="flex items-center mb-4">
        ...
        <h3 className="text-2xl font-bold text-white">ML Analysis</h3>
      </div>
      
      <div className="mb-4">
        <p className="text-white/80 text-sm mb-1">Model: {results.model}</p>
        <p className="text-white/80 text-sm">Accuracy: {(results.accuracy * 100).toFixed(1)}%</p>
      </div>

      <div className="space-y-4">
        <div>
          <h4 className="text-white font-semibold mb-2">Sales Predictions</h4>
          <ResponsiveContainer width="100%" height={150}>
            <LineChart data={results.predictions}>
              ...
              />
              <Line type="monotone" dataKey="predicted" stroke="#fbbf24" strokeWidth={2} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>

        <div>
          <h4 className="text-white font-semibold mb-2">Customer Classification</h4>
          <ResponsiveContainer width="100%" height={150}>
            <PieChart>
              <Pie
        ...
        <div>
          <h4 className="text-white font-semibold mb-2">Top Recommendations</h4>
